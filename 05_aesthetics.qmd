...
```{R libr, echo = FALSE, purl = TRUE, message=FALSE}
library(ggplot2)
library(tidyverse)
library(haven)
...
gridExtra::grid.arrange(bar_plot, bar_plot2, ncol = 2)
```
...
gridExtra::grid.arrange(ng, g, ncol = 2)
```
...
```{R, maps_notshown, fig.width = 8, fig.height = 5, echo=FALSE, purl = FALSE}
# Get some data (map of USA with state boundaries) from the map package
